...
installé par-dessus, walking skeleton exécuté avec succès (voir la sortie
en fin de guide). Un bug reel a été trouvé et corrige en route (voir §3).
...
- Méthode enregistrée sur `mecore::compute_indicators()` pour respecter
  la convention pipe du §5 : `project |> compute_indicators(recipes = ...)`.
...
jour d'`ARCHITECTURE.md` avec la règle du `zzz.R` (§3 ci-dessus) avant de
démarrer le package suivant.
